...
stateが更新されると再レンダリングが起き、再レンダリングのたびにフェッチが実行され、さらにstateが更新されるという無限ループになっています。確認できたら、ブラウザのタブを閉じます。

### 2. useEffectの基本

`src/App.jsx` を以下に書き換えます。ブラウザで `http://localhost:5173` を開きます。
...
開発環境ではマウント時に2回実行されることがあります。これはStrictModeの影響です。StrictModeはReactが開発環境でバグを検出するための仕組みで、ViteのReactテンプレートではデフォルトで有効になっています。本番環境では1回のみ実行されます。
